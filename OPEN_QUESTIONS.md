...
## P0 — Dense-31B 64K decode collapse (1.78 tok/s, AL=1.94)
...
When the SWA ring wraps (sustained generation past ~1024 tokens past the SWA window), the wrap branch in `gemma4_mtp_graph.cpp:369-388` concat-forces F32, stripping the FWHT rotation. Same class as Bug 2 (MMA intercept stripping FWHT); same fix pattern (force chunked, or split FA + combine softmax). Not hit in any current bench session because generations stayed within the unwrapped window (max 256 output tokens; SWA window = 2048). Will manifest on long chat sessions. Tracked in the journey blog.
...
## P2 — Q8/Q8 vs TQ3/TQ3 decision boundary by context length

Current data: Q8/Q8 is 2.4× faster on prefill at 64K (from journey blog lesson #9) and ~15% faster on decode at 4K (matrix-v3: 34.79 vs 30.21 tok/s N2 vs N1). TQ3/TQ3 saves ~1.3 GB VRAM at 4K. At what context length does the VRAM saving from TQ3 become worth the decode speed penalty? No ablation data above 4K for TQ3 on coherent output (all TQ3 long-ctx data pre-Bug-2 was garbage). Needs a fresh matrix-64k run post-Bug-2 with TQ3/TQ3.
